...
def tokenizer_sanity_check(tokenizer):
    ...
    tr = tokenizer(in_text, max_length=2048, pad_to_max_length=False, truncation=True)
...
def main():
    ...
    #     training_conf.local_rank = int(os.environ['LOCAL_RANK'])
    # print('local_rank:', training_conf.local_rank)
    output_dir = (
        training_conf.output_dir
        if training_conf.output_dir
        else f"{training_conf.model_name}-{training_conf.log_dir}-finetuned"
    )
    training_conf.quantization = False
    ...
    print("-------------train dataset len---------------")
    print(len(train))
    print("-------------eval dataset len---------------")
    print(len(evals))
    ...
    if training_conf.log_wandb and (not training_conf.deepspeed or training_conf.local_rank == 0):
        import wandb
        print(training_conf.wandb_entity)
